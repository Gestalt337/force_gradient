...
class Cell:
    ...
    def addPolygon(self, polygon):
        ...
        for polyvertices in polygon.vertices_:
            if polyvertices not in self.vertices_:
...
def dumpTopo(vertices, edges, polygons, cells, points, Lxyz):
    Lx, Ly, Lz = Lxyz
    with open("data/vertices.csv", "w") as file:
        ...
        # count = 0
    with open("data/polygons.csv","w") as file:
        for key in polygons:
            polygon = polygons[key]
    ...
    with open("data/polygon.csv", "w") as file:
        ...
        #file.write("cells {:d}\n".format(len(cells)))
        # count = 0
    with open("data/cell.csv", "w") as file:
        for key in cells:
            cell = cells[key]
            file.write("Cell, "+str(key)+"\n")
